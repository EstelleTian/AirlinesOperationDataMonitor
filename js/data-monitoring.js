      ...
      success: function (data) {
        if ($.isValidObject(data)) {
          ...
          if (totalDataCount == null) {
            ...
            for (var x in data.totalDataCount) {
              if (data.totalDataCount[x] === 'undefined' || data.totalDataCount[x] === '' || data.totalDataCount[x] === 'NAN') {
                data.totalDataCount[x] = '-'
              }
            }
            refreshData(data)
          }
    ...
    $.ajax({
      ...
      success: function (data) {
        if ($.isValidObject(data)) {
          ...
          var newDataLength = Object.keys(airportsData)
          if (airportsChartArr.dataArr.length == newDataLength.length) {
            var len = airportsChartArr.airNumOptions.length
            var airportsDataArr = [];
            $.each(airportsData,function (i,e) {
              airportsDataArr.push(e)
            })
            for (var i = 0; i < len; i++) {
              airportsDataArr[i].currentTime = generateTime
              airportsChartArr.airNumOptions[i].refreshOption(airportsDataArr[i],airportChartOpt, 'data_COUNT')
              airportsChartArr.numChartArr[i].setOption(airportsChartArr.airNumOptions[i])
              airportsChartArr.airFileOptions[i].refreshOption(airportsDataArr[i], airportChartOpt, 'file_COUNT')
            ...
            for (var i = 0; i < len; i++) {
              for (var j = 0; j < airportsData.length; j++) {
                airportsData[i].currentTime = generateTime
                airportsChartArr.airNumOptions[i].refreshOption(airportsData[i], airportChartOpt,'data_COUNT')
                airportsChartArr.numChartArr[i].setOption(airportsChartArr.airNumOptions[i])
                airportsChartArr.airFileOptions[i].refreshOption(airportsData,airportChartOpt, 'file_COUNT' )
                airportsChartArr.fileChartArr[i].setOption(airportsChartArr[i].airFileOptions[i])
              }
            }
          }
    ...
    $.ajax({
      ...
      success: function (data) {
        if ($.isValidObject(data)) {
          ...
          var newDataLength = Object.keys(companyDatas)
          if (companyChartsArr.dataArr.length == newDataLength.length) {
            var len = companyChartsArr.comFileOptions.length
            var companyDatasArr = [];
            $.each(companyDatas,function (i,e) {
              companyDatasArr.push(e)
            })
            for (var i = 0; i < len; i++) {
              companyDatasArr[i].currentTime = generateTime
              companyChartsArr.comNumOptions[i].refreshOption(companyDatasArr[i], companyChartOpt, 'data_COUNT')
              companyChartsArr.numChartArr[i].setOption(companyChartsArr.comNumOptions[i])
              companyChartsArr.comFileOptions[i].refreshOption(companyDatasArr[i], companyChartOpt,'file_COUNT')
            ...
            for (var i = 0; i < len; i++) {
              for (var j = 0; j < companyDatas.length; j++) {
                if (companyDatas[j].airportName == companyChartsArr.dataArr[i].airportName) {
                  ...
                  var numOptions = new CompanyOptions(companyDatas[i], companyChartOpt, 'data_COUNT')
                  companyChartsArr.comNumOptions.push(numOptions)
                  var fileOptions = new CompanyOptions(companyDatas[i], companyChartOpt, 'file_COUNT')
                  companyChartsArr.comFileOptions.push(fileOptions)
                  var chartsMulNum = echarts.init($('#flight_num' + j)[0])
                  var chartsMulFile = echarts.init($('#flight_file' + j)[0])
                  companyChartsArr.numChartArr.push(chartsMulNum)
            ...
            for (var i = 0; i < len; i++) {
              for (var j = 0; j < companyDatas.length; j++) {
                companyDatas[i].currentTime = generateTime
                companyChartsArr.comNumOptions[i].refreshOption(companyDatas[i], companyChartOpt, 'data_COUNT')
                companyChartsArr.numChartArr[i].setOption(companyChartsArr.comNumOptions[i])
                companyChartsArr.comFileOptions[i].refreshOption(companyDatas[i], companyChartOpt,'file_COUNT')
                companyChartsArr.fileChartArr[i].setOption(companyChartsArr.comFileOptions[i])
              }
            }
          }
